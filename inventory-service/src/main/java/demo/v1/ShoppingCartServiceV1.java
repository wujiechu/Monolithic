...
public class ShoppingCartServiceV1 {
    ...
    private CatalogServiceV1 catalogServiceV1;
    private RestTemplate restTemplate;

    @Autowired
    public ShoppingCartServiceV1(CartEventRepository cartEventRepository,
                                 CatalogServiceV1 catalogServiceV1,
    ...
    public ShoppingCart getShoppingCart() throws Exception {
        ...
        if (user != null) {
            ...
            Catalog catalog = catalogServiceV1.getCatalog();
    ...
    public CheckoutResult checkout() throws Exception {
        ...
        if (currentCart != null) {
            ...
            /*Inventory[] inventory =
             ...
             List<Inventory> inventories = inventoryRepository.getAvailableInventoryForProduct(currentCart.getLineItems()
                    .stream()
                    .map(LineItem::getProductId)
                    .collect(Collectors.joining(",")));
            Inventory[] inventory  = inventories.toArray(new Inventory[0]);
            if (inventory != null) {
